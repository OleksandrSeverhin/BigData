...
# Prepare matrices for glmnet (It requires Matrix format, not DataFrames)
...
# A. Baseline OLS
ols_model <- lm(fare_amount ~ ., data = train_processed)
...
results$pred_ols <- predict(ols_model, newdata = test_processed)

results$pred_ridge <- as.vector(predict(cv_ridge, newx = x_test, s = lambda_ridge))
results$pred_lasso <- as.vector(predict(cv_lasso, newx = x_test, s = lambda_lasso))
results$pred_enet  <- as.vector(predict(cv_enet, newx = x_test, s = lambda_enet))
...
coef_df <- data.frame(
  ...
  # Match names carefully, defaulting to NA if not found
...
coef_plot_long <- coef_df %>%
  pivot_longer(cols = -Term, names_to = "Model", values_to = "Coefficient") %>%
